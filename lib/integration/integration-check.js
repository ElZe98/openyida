...
function worksheetXml(rows) {
  ...
  return [
    ...
    `<dimension ref="A1:${columnName(EXPORT_HEADERS.length)}${Math.max(rows.length, 1)}"/>`,
...
function buildAppRows(appType, abnormalFlows, errors = []) {
  ...
  if (appError) {
    rows.push([appType, '', '', '', '', '', '', '', '', '', 0, '', '', '', `应用检查失败：${appError.message}`, '', '', '']);
  }
  if (!flows.length) {
    if (!appError) {
      rows.push([appType, '', '', '', '', '', '', '', '', '', 0, '', '', '', '未发现执行异常日志', '', '', '']);
    }
    return rows;
  }
...
function buildXlsxWorkbook(sheets) {
  ...
  const normalizedSheets = sheets.map((sheet, index) => ({
    ...
    rows: sheet.rows && sheet.rows.length ? sheet.rows : [EXPORT_HEADERS],
  }));
...
function exportResultToExcel(result, outputPath) {
  ...
  const workbook = buildXlsxWorkbook(sheets.length ? sheets : [{ name: '结果', rows: [EXPORT_HEADERS] }]);
...
async function run(args) {
  if (!args.length || hasFlag(args, '--help') || hasFlag(args, '-h')) {
    usage(
      ...
      'openyida integration check APP_XXX --output project/output/自动化异常.xlsx'
    );
    process.exit(0);
  }
    ...
    try {
      ...
      const appResult = await collectAbnormalFlows(authRef, appType, {
        ...
        onProgress: ({ current, total, flow }) => {
          ...
          progress.update(`检查进度 ${formatProgressBar(current, total)}${suffix}`);
        },
      });
      progress.finish();
  ...
  if (outputJson) {
    ...
    if (outputPath) {
      ...
      success(`Excel 已导出：${exportedPath}`);
    }
    return;
  }
