...
const { refreshGraphForApp } = require('../graph-auto-update');
...
async function run(args) {
  ...
  const prevFlag = process.env.OPENYIDA_SKIP_GRAPH_UPDATE;
  try {
    const { run: runConfigureProcess } = require('./configure-process');
    process.env.OPENYIDA_SKIP_GRAPH_UPDATE = '1';
    ...
    if (typeof prevFlag === 'undefined') {
      delete process.env.OPENYIDA_SKIP_GRAPH_UPDATE;
    } else {
      process.env.OPENYIDA_SKIP_GRAPH_UPDATE = prevFlag;
    }
  }
  ...
  await refreshGraphForApp(appType, 'create-process');
